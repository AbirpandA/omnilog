...
export function DetailsScreen({ route, navigation }: Props) {
  const { media_id, title, description, poster_url, similarity_score, vibe_tag } = route.params;
            ...
            <View style={styles.titleContainer}>
              {similarity_score && (
                <View style={styles.scoreBadge}>
                  ...
                  <Text style={styles.scoreText}>{(similarity_score * 100).toFixed(0)}% Match</Text>
                </View>
              )}
              <Text style={styles.title}>{title}</Text>
              {details && (
                <Text style={styles.subtitle}>
                  {details.release_date ? details.release_date.split('-')[0] : ''} • {details.runtime}m
                </Text>
              )}
              {details?.tmdb_rating ? (
                <View style={styles.tmdbRatingContainer}>
                  <Star color="#f5c518" size={16} fill="#f5c518" />
                  <Text style={styles.tmdbRatingText}>{details.tmdb_rating.toFixed(1)} / 10</Text>
                </View>
              ) : null}
            </View>
          </View>
          ...
          <View style={styles.actionRow}>
            ...
            <TouchableOpacity 
              style={[
                ...
                localLog && ReactionConfig[localLog.reaction as ReactionType] && { borderColor: ReactionConfig[localLog.reaction as ReactionType].color, backgroundColor: 'rgba(255,255,255,0.05)' }
              ]} 
            ...
            >
              {localLog && ReactionConfig[localLog.reaction as ReactionType] ? (
                <>
                  {React.createElement(
                    ReactionConfig[localLog.reaction as ReactionType].icon || Star, 
                    { color: ReactionConfig[localLog.reaction as ReactionType].color, size: 24 }
                  )}
                  <Text style={[styles.primaryActionText, { color: ReactionConfig[localLog.reaction as ReactionType].color }]}>
            ...
            <View style={styles.secondaryActionGroup}>
              <TouchableOpacity 
                style={[styles.iconButton, localLog?.reaction === 'watchlist' && styles.iconButtonActive]}
                onPress={() => handleToggleSecondary('watchlist')}
              >
                <Bookmark color={localLog?.reaction === 'watchlist' ? '#33b5e5' : '#888'} size={22} />
              </TouchableOpacity>
              
              <TouchableOpacity 
                style={[styles.iconButton, localLog?.reaction === 'collection' && styles.iconButtonActive]}
                onPress={() => handleToggleSecondary('collection')}
              >
                <FolderHeart color={localLog?.reaction === 'collection' ? '#ff8800' : '#888'} size={22} />
              </TouchableOpacity>
            </View>
          </View>
        ...
        <BlurView intensity={70} tint="dark" style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle} numberOfLines={1}>Log "{title}"</Text>
...
const styles = StyleSheet.create({
  ...
  heroGradient: { ...StyleSheet.absoluteFill, backgroundColor: 'rgba(5,5,5,0.4)' },
  heroOverlay: { position: 'absolute', bottom: 0, left: 0, right: 0, height: 150, backgroundColor: 'rgba(5,5,5,0.5)' },
  backButton: { position: 'absolute', top: 60, left: 20, borderRadius: 24, overflow: 'hidden', zIndex: 10 },
  backButtonBlur: { padding: 10, borderRadius: 24, backgroundColor: 'rgba(255,255,255,0.05)' },
  ...
  overlayPoster: { width: 110, height: 165, borderRadius: 16, borderWidth: 2, borderColor: '#222', backgroundColor: '#111' },
  vibeTag: { position: 'absolute', bottom: -8, alignSelf: 'center', backgroundColor: 'rgba(0,0,0,0.8)', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 12, borderWidth: 1, borderColor: '#333' },
  ...
  scoreBadge: { flexDirection: 'row', alignItems: 'center', backgroundColor: 'rgba(255, 215, 0, 0.1)', paddingHorizontal: 12, paddingVertical: 4, borderRadius: 12, alignSelf: 'flex-start', marginBottom: 8 },
  ...
  actionRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 24, gap: 12, zIndex: 20 },
  primaryActionButton: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: '#222', paddingVertical: 14, borderRadius: 20, borderWidth: 1, borderColor: '#333' },
  primaryActionText: { color: '#fff', fontWeight: 'bold', fontSize: 16, marginLeft: 8 },
  secondaryActionGroup: { flexDirection: 'row', gap: 12 },
  iconButton: { backgroundColor: '#1a1a1a', padding: 14, borderRadius: 20, borderWidth: 1, borderColor: '#333', alignItems: 'center', justifyContent: 'center' },
  iconButtonActive: { backgroundColor: 'rgba(255,255,255,0.05)', borderColor: 'rgba(255,255,255,0.2)' },
  ...
  tag: { backgroundColor: '#1a1a1a', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 12, marginRight: 8, marginBottom: 8, borderWidth: 1, borderColor: '#333' },
  tagText: { color: '#aaa', fontSize: 12, fontWeight: '600' },
  
  sectionTitle: { fontSize: 20, fontWeight: '700', color: '#ffffff', marginBottom: 12, marginTop: 8 },
  description: { fontSize: 16, color: '#a0a0a0', lineHeight: 24, marginBottom: 16 },

  horizontalFlashListContainer: { height: 210, marginHorizontal: -24, paddingHorizontal: 24 },
  horizontalCard: { width: 120, marginRight: 16 },
  horizontalPoster: { width: 120, height: 180, borderRadius: 16, backgroundColor: '#222', marginBottom: 8 },
  ...
  modalContent: { backgroundColor: '#111', borderTopLeftRadius: 32, borderTopRightRadius: 32, padding: 24, paddingBottom: 40, borderWidth: 1, borderColor: '#222' },
  ...
  modalSubHeader: { color: '#888', fontSize: 13, textTransform: 'uppercase', fontWeight: 'bold', marginBottom: 16 },
