...
export function TasteProfileScreen({ navigation }: Props) {
  ...
  const fetchLogDetails = async () => {
    ...
    const topLogs = logs.filter(
      (l) => l.reaction === "pure gold" || l.reaction === "Absolute cinema",
    );
    // Fetch details for up to 10 top movies
    const detailed = await Promise.all(
      topLogs
        .slice(0, 10)
        .map((l) => fetchMediaDetails(l.mediaId).catch(() => null)),
    );
  ...
  const hasMasterCritic =
    logs.filter((l) => l.reaction === "Absolute cinema").length >= 5;
  const hasJournalist =
    logs.filter((l) => l.notes && l.notes.trim().length > 0).length >= 5;
        ...
        {isLoading ? (
          ...
          <>
            ...
            <GlassCard style={styles.personaCard}>
              ...
              <Text style={styles.personaValue}>
                {generatePersona(logDetails)}
              </Text>
              <Text style={styles.personaDesc}>
                Based on the movies you rated highest.
              </Text>
            </GlassCard>
            ...
            <View style={{ marginBottom: 24 }}>
              <View style={styles.personaHeader}>
                ...
                <Text style={[styles.personaTitle, { fontSize: 22 }]}>
                  Achievements
                </Text>
              </View>
              <ScrollView
              ...
              >
                ...
                <View
                  style={[
                    ...
                    !hasFirstStep && styles.badgeLocked,
                  ]}
                >
                  {hasFirstStep ? (
                    <Award color="#b829ea" size={32} />
                  ) : (
                ...
                <View
                  style={[
                    styles.badgeContainer,
                    !hasMasterCritic && styles.badgeLocked,
                  ]}
                >
                  {hasMasterCritic ? (
                    <Award color="#00C851" size={32} />
                  ) : (
                    <Lock color="#666" size={24} />
                  )}
                ...
                <View
                  style={[
                    styles.badgeContainer,
                    !hasJournalist && styles.badgeLocked,
                  ]}
                >
                  {hasJournalist ? (
                    <Award color="#ffbb33" size={32} />
                  ) : (
                    <Lock color="#666" size={24} />
                  )}
            ...
            <View style={{ marginTop: 16 }}>
              ...
              {logs.filter((l) => l.notes && l.notes.trim().length > 0).length >
              0 ? (
                logs
                ...
                <GlassCard style={styles.journalCard}>
                  <Text style={styles.journalPlaceholder}>
                    You haven't written any journal entries yet. Go to a movie
                    details screen and add your thoughts!
                  </Text>
                </GlassCard>
              )}
            </View>
          ...
          <View style={styles.emptyContainer}>
            <Text style={styles.emptyText}>
              Rate some movies to generate your Taste Profile!
            </Text>
          </View>
        )}
      </ScrollView>
