...
class SupabaseMediaProvider(IMediaProvider):
    ...
    async def get_candidates_for_vibe(
        ...
        if not self.client:
            ...
            return await self.tmdb_provider.get_candidates_for_vibe(
                seed_movies, exclude_ids, limit
            )
    ...
    async def get_candidates_for_mood(
        ...
        if not self.client:
            return await self.tmdb_provider.get_candidates_for_mood(
                mood_text, exclude_ids, limit
            )
    ...
    async def get_candidates_for_similar(
        ...
        res = await run_in_threadpool(_fetch_seed)
        ...
        if res.data and len(res.data) > 0 and res.data[0].get("embedding"):
            ...
            tmdb_res = await self.tmdb_provider.get_media_by_ids([media_id])
            if not tmdb_res:
                return []
    ...
    async def get_latest_movies(self) -> List[CandidateMedia]:
        return await self.tmdb_provider.get_latest_movies()

    async def get_upcoming_movies(self) -> List[CandidateMedia]:
        return await self.tmdb_provider.get_upcoming_movies()

    async def search_media(self, query: str) -> List[CandidateMedia]:
        return await self.tmdb_provider.search_media(query)

    async def get_media_by_ids(self, ids: List[str]) -> List[CandidateMedia]:
        return await self.tmdb_provider.get_media_by_ids(ids)

    async def get_movie_details(self, movie_id: str) -> Optional[dict]:
        return await self.tmdb_provider.get_movie_details(movie_id)
