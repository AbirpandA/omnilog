...
class SyncMoviesUseCase:
    ...
    def __init__(
        self, tmdb_provider: RealTMDBProvider, supabase_provider: SupabaseMediaProvider
    ):
    ...
    async def sync_trending_movies(self):
        ...
        try:
            latest = await self.tmdb.get_latest_movies()
            upcoming = await self.tmdb.get_upcoming_movies()
            ...
            for candidate in all_candidates:
                ...
                res = (
                    self.supabase.client.table("movies")
                    .select("id")
                    .eq("id", candidate.id)
                    .execute()
                )
            ...
            logger.info(
                f"Background sync complete. Added {inserted} new movies to Deep Catalog."
            )
